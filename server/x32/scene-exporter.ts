/**
 * X32 Scene Exporter
 * Creates USB-compatible .scn scene files by querying all console parameters via OSC
 *
 * The X32 .scn format is plain ASCII text containing ~2000+ lines of OSC address/value pairs.
 * Format: #firmware# "SceneName" "Notes" safetymask hasaliases
 *
 * This module uses the /node OSC command to efficiently query parameter groups
 * rather than individual parameters.
 *
 * References:
 * - Patrick-Gilles Maillot's X32 utilities: https://github.com/pmaillot/X32-Behringer
 * - Unofficial X32/M32 OSC Remote Protocol documentation
 */
...
export interface ExportOptions {
  ...
  port?: number;
  /** Scene name to use in header */
  sceneName: string;
  /** Scene notes */
  notes?: string;
  /** Query timeout in ms (default: 1000) */
  timeout?: number;
  /** Progress callback */
...
/**
 * Create a simple OSC query (no arguments)
 */
...
/**
 * X32 Scene Exporter class
 * Queries all console parameters using /node and generates a USB-compatible .scn file
 */
export class X32SceneExporter {
  ...
  /**
   * Export all console parameters to .scn format
   */
  async exportScene(options: ExportOptions): Promise<ExportResult> {
    ...
    const totalNodes = NODE_PATHS.length;
    const lines: string[] = [];

    // Add header
    lines.push(`#${consoleInfo.firmware}# "${options.sceneName}" "${options.notes || ''}" %0000000 0`);
    ...
    for (const nodePath of NODE_PATHS) {
      ...
      // Report progress
      if (options.onProgress && (completed % 100 === 0 || completed === totalNodes)) {
        const section = completed < 400 ? 'Config' :
                        completed < 1200 ? 'Channels' :
                        completed < 1400 ? 'Aux/FX' :
                        completed < 1700 ? 'Buses' :
                        'Outputs';
...
/**
 * Export a complete scene from the X32 to .scn format
 * Convenience function that handles connection lifecycle
 */
export async function exportSceneToScn(options: ExportOptions): Promise<ExportResult> {
  const exporter = new X32SceneExporter(
    options.ip,
    options.port || 10023,
  ...
  try {
    ...
    return await exporter.exportScene(options);
  } finally {
    exporter.close();
  }
}
